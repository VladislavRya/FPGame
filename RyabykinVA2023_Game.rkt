...
;; broom
;;
;; a broom owner can move at any room regardless of exits
;; broom owner can not move to special rooms (e.g. heaven and spell-room)
...
(define person%
   (class mobile-thing%
      ...
      (define/public (suffer hits perp) (send this say (list "Ouch!" hits "hits is more than I want!"))
        ; attack troll back
        (begin (set! health (- health hits)) (if (and (> health 0) (is-a? perp troll%)) (begin (send this say (list "Get it stupid troll!" (send perp get-name))) (send perp suffer (random-number strength) this) (send faculty add-score-for-attack! this)) #f))
        (when (<= health 0) (send this die perp)) health)
      (define/public (suffer-from-spell damage spell-name perp) (send this say (list "Magic" spell-name "is not applicable to me")))
...
(define autonomous-person%
   (class person%
      ...
      (define/public (move-somewhere)
        ; if has broom then can move anywhere wants (except heaven and chamber-of-stata)
        (if (have-broom? this) (let ((new-location (pick-random all-visitable-rooms)))
             (begin (send this say '("I Can use the power of broom! Yoouhooooo!"))
...
(define hall-monitor% 
   (class autonomous-person%
     ...
     (define/override (suffer-from-spell damage spell-name perp) (begin (send this say (list "Ouch! Your magic" spell-name "hits me" damage "it hurts!"))
...
(define troll% 
   (class autonomous-person%
	  ...
	  (define/override (die perp) (begin
              ...
              (super die perp)))
     (define/override (suffer-from-spell damage spell-name perp) (begin (send this say (list "ARGHHH I HATE MAGIC!!! It hits me" damage "damage"))
                                                                        (set-field! health this (- (get-field health this) damage)) (when (<= (get-field health this) 0) (send this die perp)) (get-field health this)))
     (super-new (name name) (birthplace birthplace) (activity activity) (miserly miserly))
    ))
...
(define dementor% 
   (class troll%
     ...
     (define/override (suffer hits perp) (send this say '("Look how stupid you are! You are trying to attack me, DEMENTOR! I am INVINCIBLE WU-A-HA-HA")))
     (define/override (suffer-from-spell damage spell-name perp) (send this say '("Ha-ha I am DEMENTOR. Your spells do nothing to me!"))
                                                                        (if (eq? spell-name 'patronus) (begin (send this say (list "Oh NOOOO this is patronus!! It hits me" damage "damage"))
                                                                        (set-field! health this (- (get-field health this) damage)) (when (<= (get-field health this) 0) (send this die perp))) (get-field health this)))
...
(define professor% 
   (class autonomous-person%
     (init name birthplace activity miserly)
     ; professor knows one spell picked randomly
     ; professor creates a copy of spell and gives it to a person
     ...
     (define/public (teach-spell)
         ;professor teaches each person around him a spell he knows
	     (let ((people (send this people-around)))
	      (if (not (null? people))
		  (begin
		    (send this say '("Hello dear young magica enjoyers. I will teach you something incredible!"))
		    (for-each (lambda (person) (begin
                                                 (send this say (list "I teach" (send person get-name) "a" (send this get-known-spell) "spell"))
                                                 (clone-spell (get-spell-by-name (send this get-known-spell)) person)))
			      people)
		    'burum-magica-burum)
		    (send this say '("I want to share my knowledge, but nobody listens to me :("))))
	     (when (send this people-around)
		   (send this say '("I love magic so much, my life would be horrible without it!"))))
     (define/override (die perp) (begin
...
(define chosen-one% 
   (class autonomous-person%
     ...
     (define/override (suffer hits perp) (send this say (list "Ouch!" hits "hits is more than I want!"))
       ; chosen one doen't die on critical damage. He receives 0 damage and kills his enemy 
       (if (< (- (send this get-health) hits) 0) (begin (send this say '("You thought I would die? NO! But you WILL!")) (send perp die this))
       ; attack troll back if not critical damage
       (begin (set-field! health this (- (get-field health this) hits)) (if (is-a? perp troll%) (begin (send this say (list "Get it stupid troll!" (send perp get-name))) (send perp suffer (random-number (get-field strength this)) this) (send (send this get-faculty) add-score-for-attack! this)) #f))))	   
     (super-new (name name) (birthplace birthplace) (activity activity) (miserly miserly))
))
...
(define avatar%
   (class person%
	  ...
	  (send screen tell-world (if (null? other-people) '("There are no other people around you.") (append '("You see other people:") (names-of other-people))))
          (send screen tell-world (if (null? broom-visitable-places) '("Maybe you are dead... But maybe you just simply don't have a broom. Time will tell") (append '("You have a broom and can visit this places (use go-place <placename>):") (names-of broom-visitable-places))))
     ...
     (define/public (go-place place-name) (if (have-broom? this)
        ; move using broom
        (let ((new-place (get-place-by-name place-name))) (if (null? new-place) (send screen tell-world (list "You can not go to" place-name))
            (begin (send this say '("I Can use the power of broom! Yoouhooooo!"))
        ...
        ; a-ta-ta
        (send screen tell-world (list "A player" (send this get-name) "tried to cheat and go to" place-name "without having a broom. He really made me upset"))))
      (super-new (name name) (birthplace birthplace))
     ))  
...
;; faculty
;; 
;; define faculties (actually 3 "real" faculties and one special faculty - no-faculty)
;; each faculty gives different amount of faculty-score for different students' actions
;; students of the faculty are defined at the start of the game (but this behaviour may change in future releases of the game!)
...
(define faculty%
   (class named-object%
      ...
      (define/public (add-score-for-being-chosen-one! student) (if (is-a? student chosen-one%) (send student add-faculty-score! 5) (send screen tell-world (list "Sorry" (send student get-name) "but you are not the chosen-one. No scores for you!"))))
      (super-new (name name))
     ))
...
(define griffinmath%
   (class faculty%
     ...
     (define/override (add-score-for-being-chosen-one! student) (if (is-a? student chosen-one%) (send student add-faculty-score! 60) (send screen tell-world '("Griffinmath without chosen-one is like Griffindor without Harry Potter"))))
     (super-new (name "griffinmath") (description "Dambldor's... I mean Rector's favourites"))
   ))
...
(define vmikkerpuff%
   (class faculty%
     ...
     (super-new (name "vmikkerpuff") (description "The faculty of smart and talented people. Appreciates students for practising combat magic"))
   ))
...
(define phystherin%
   (class faculty%
     ...
     (define/override (add-score-for-attack! student) (send student add-faculty-score! 4))
     (super-new (name "phytherin") (description "The mighty faculty of phystherin. Rewards his students for attacking enemies"))
   ))

; no-faculty is used to show that person does not have a faculty (incoming addon with more faculties??)
(define no-faculty%
   (class faculty%
     ...
     (super-new (name "no-faculty") (description "This is not a real faculty, sorry :("))
   ))

;; also define a leaderboard to show top of faculties and students
;; leaderboard
;;
...
(define leaderboard%
  (class object%
    ...
    (define/public (print-top-faculties) (begin (send screen tell-world (list "===============" "TOP OF FACULTIES"))
                                                (for-each (lambda (faculty) (send screen tell-world (list (send faculty get-name) ":" (send faculty get-total-score)))) (get-sorted-faculties-by-score))
                                                (send screen tell-world (list "==============="))))
    ...
    (define/public (print-top-students) (begin (send screen tell-world (list "===============" "TOP OF STUDENTS"))
                                                (for-each (lambda (student) (send screen tell-world (list (send student get-name) ":" (send student get-faculty-score)))) (get-sorted-students-by-score))
                                                (send screen tell-world (list "==============="))))
    (super-new)
  )
)
...
(define (create-world)
  ...
  (let* ((10-250 (new place% (name '10-250)))
    ...
    ; broom can also be created at this stage, but I prefer dynamic generation
...
; spawn brooms at some rooms
(define (populate-brooms rooms)
  (for-each (lambda (room)
              (if (>= (random-number 7) 6) (new broom% (origin room)) #f))
              rooms))

; create faculties
(define griffinmath (new griffinmath%))
(define vmikkerpuff (new vmikkerpuff%))
(define phystherin (new phystherin%))
...
; yes, this is a sorting hat. Each student gets a faculty
(define (use-sorting-hat)
 (send griffinmath add-student! (get-student-by-name 'hairy-cdr))
 (send vmikkerpuff add-student! (get-student-by-name 'alyssa-hacker))
 (send vmikkerpuff add-student! (get-student-by-name 'lambda-man))
 (send phystherin add-student! (get-student-by-name 'ben-bitdiddle))
 (send phystherin add-student! (get-student-by-name 'course-6-frosh))
)
...
(send leaderboard set-faculties! (list griffinmath vmikkerpuff phystherin))
...
(define (setup name)
  ...
  (let ((rooms (create-world)))
    ...
    (let ((r (pick-random rooms))) (new chosen-one% (name 'hairy-cdr) (birthplace r) (activity (random-number 3)) (miserly (random-number 3))))
...
(setup 'ben-bitdiddle)
; in my opinion this game can be played without faculties
(use-sorting-hat)
; of course you can give the chosen-one some extra score, why not?
(send griffinmath add-score-for-being-chosen-one! (get-student-by-name 'hairy-cdr))
; even twice
(send griffinmath add-score-for-being-chosen-one! (get-student-by-name 'hairy-cdr))
(run-clock 25)
